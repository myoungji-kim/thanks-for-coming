...
class paging{
		...
		*/
        ...
        function pagelink($limit,$block,$page,$t_cnt, $url, $pagevalue="", $PRM_2) {
	...
	//			echo "<div class='pagecont'>";
				...
				if($page != 1) {
					// echo '<li><a href="'.$url.$tmpflag.$pagevalue.'='."1".'">처음</a></li>';
					echo '<a href="'.$url.$tmpflag.$pagevalue.'='.($page-1).'"><</a>';
				} else {
				...
				if ($lo_block>$sum_page || $lo_block==$sum_page)
				{
						...
						{
							$tmp_cur = "";
							if ($page==$i) $tmp_cur = " class='NowPagePoint' ";
							echo '<a '.$tmp_cur.'href="'.$url.$tmpflag.$pagevalue."=".$i.$tmpflag_2.$PRM_2.$SearchWord.'">'.$i.'</a>';
						}
				} else {
				...
				if($sum_page > $page) {
					echo '<a href="'.$url.$tmpflag.$pagevalue.'='.($page+1).'">></a>';
					// echo '<li><a href="'.$url.$tmpflag.$pagevalue.'='.$sum_page.'">마지막</a></li>';
				} else {
					echo '<a href="javascript://">></a>';
				}
